...
$stmt = $conn->prepare("SELECT * FROM po_users WHERE id = :user_id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['profile_update'])) {
    ...
    if (empty($name) ||  empty($email) || empty($username)) {
        ...
        $stmt = $conn->prepare("UPDATE po_users SET name = :name, email = :email, username = :username WHERE id = :user_id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['change_pass'])) {
    ...
    $stmt = $conn->prepare("SELECT password FROM po_users WHERE id = :id");
    ...
    if ($user && password_verify($current_password, $user['password'])) {
        if ($new_password === $confirm_password) {
            if (!preg_match('/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $new_password)) {
                ...
                $stmt = $conn->prepare("UPDATE po_users SET password = :password WHERE id = :id");
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_type, ['image/jpeg', 'image/png', 'image/gif']) && $file_size <= 1048576) { 
            ...
            if (move_uploaded_file($file_tmp_name, $target_file)) {
                ...
                $stmt = $conn->prepare("UPDATE po_users SET profile_picture = :profile_picture WHERE id = :user_id");
