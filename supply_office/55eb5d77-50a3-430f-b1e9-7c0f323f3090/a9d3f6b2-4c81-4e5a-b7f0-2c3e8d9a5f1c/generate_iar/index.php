    ...
    <main class="container mt-5 custom-container ">
        ...
        <form id="inspectionForm" method="POST">
            <section class="form-section" id="section1">
                ...
                <div class="input-group">
                        ...
                        <input type="text" id="requestor" name="requestor" value="<?php echo htmlspecialchars($poData['requestor'] ?? ''); ?>" required readonly>
                    </div>
                    <div class="input-group">
                        <label for="fund_cluster">Fund Cluster</label>
                        <input type="text" id="fund_cluster" name="fund_cluster" value="<?php echo htmlspecialchars($poData['fund_cluster'] ?? ''); ?>" >
                    </div>
                </div>
                <div class="grid-2">
                    <div class="input-group">
                        <label for="supplier">Supplier</label>
                        <input type="text" id="supplier" name="supplier" value="<?php echo htmlspecialchars($poData['supplier'] ?? ''); ?>" required readonly>
                    </div>
                    <div class="input-group">
                        <label for="iar_no">IAR No.</label>
                        <input type="text" id="iar_no" name="iar_no" value="<?php echo htmlspecialchars($iar_no); ?>" required readonly>
                    </div>
                </div>
                <div class="grid-2">
                    <div class="input-group">
                        ...
                        <?php
                        $poDate = !empty($poData['date']) ? new DateTime($poData['date']) : '';
                        $formattedDate = $poDate ? $poDate->format('F j, Y') : '';
                        ?>
                        <input type="text" id="po_no" name="po_no" 
                            value="<?php echo htmlspecialchars($poData['po_no'] ?? ''); ?> | <?php echo $formattedDate; ?>" 
                            required readonly>
                    </div>
                    <div class="input-group">
                ...
                <div class="grid-2">
                    <div class="input-group">
                        ...
                        <input type="text" id="req_office" name="req_office" value="<?php echo htmlspecialchars($poData['requisitioning_office'] ?? ''); ?>" readonly required>
                    </div>
                    <div class="input-group">
                        <label for="invoice_no">Invoice No.</label>
                        <input type="text" id="invoice_no" name="invoice_no" placeholder="Type..." required>
                    </div>
                </div>
                <div class="grid-2">
                <div class="input-group">
                    ...
                    <div class="searchable-select" id="customSelect">
                        <input type="text" class="search-box" placeholder="Select or Search..." id="searchBox" onkeyup="filterItems()" onclick="toggleDropdown()" autocomplete="off" required>
                    ...
                    <select id="resp_code" name="resp_code" style="display: none;" required >
                        <option value="">Select Code</option>
                        <?php foreach ($uacsCodes as $code): ?>
                            <option value="<?php echo htmlspecialchars($code['uacs']); ?>"><?php echo htmlspecialchars($code['uacs']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                    <div class="input-group">
        ...
        <div class="table-responsive" >
            ...
            <table class="table">
                ...
                foreach ($remainingItems_1 as $item): ?>
                    <?php if (isset($labelsByNo[$newStockNo])): ?>
                        <?php foreach ($labelsByNo[$newStockNo] as $label): ?>
                            <tr>
                                <td colspan="6"><?php echo htmlspecialchars($label); ?></td>
                            </tr>
                            <input type="hidden" name="label_no[]" value="<?php echo htmlspecialchars($newStockNo); ?>">
                            <input type="hidden" name="labels[]" value="<?php echo htmlspecialchars($label); ?>"> 
                        <?php endforeach; ?>
                    <?php endif; ?>
                    <tr id="row-<?php echo htmlspecialchars($newStockNo); ?>"> 
                        <td><?php echo htmlspecialchars($newStockNo); ?></td>
                        ...
                        <td class="item-status" id="status-<?php echo htmlspecialchars($newStockNo); ?>"></td>
                        <td>
                        <?php if ($item['is_arrived']): ?>
                            <a href="#" class="inspect-icon" data-toggle="modal" 
                                data-stock-no="<?php echo htmlspecialchars($newStockNo); ?>" 
                                data-description="<?php echo htmlspecialchars($item['description']); ?>" 
                                data-unit="<?php echo htmlspecialchars($item['unit']); ?>" 
                                data-quantity="<?php echo htmlspecialchars($item['received_quantity']); ?>"
                                title="Inspect">
                    ...
                    <!-- Display item specifications in a new row -->
                    <?php 
                    ...
                    foreach ($specifications as $spec): ?>
                        <tr>
                            <td></td> <!-- Empty for Stock/Property No. --> 
                            <td><?php echo htmlspecialchars($spec['item_specification']); ?>
                            <td></td> 
                            <td></td> 
                            <td></td> 
                            <td></td> 
                        </tr>
                        <input type="hidden" name="item_no[]" value="<?php echo htmlspecialchars($newStockNo); ?>">
            ...
            <section class="form-section" id="section3" style="display:none;">
                <h3>Inspection Conforme</h3>
                    ...
                    <?php
                        $sql = "SELECT name FROM office WHERE role = 'Property Custodian'";
                        $stmt = $conn->prepare($sql);
                        $stmt->execute();

                        $propertyCustodianName = ""; 
                        if ($stmt->rowCount() > 0) {
                            $row = $stmt->fetch(PDO::FETCH_ASSOC);
                            $propertyCustodianName = $row['name'];
                            $propertyCustodianName = htmlspecialchars($propertyCustodianName, ENT_QUOTES, 'UTF-8');
                        }
                        ?>
                   ...
                   <div class="grid-2">
                    <div class="input-group">
                        ...
                        <input type="hidden" id="head_procurement" name="head_procurement" value="<?php echo htmlspecialchars($propertyCustodianName, ENT_QUOTES, 'UTF-8'); ?>" readonly> 
                    </div>
                </div>
                    <p class="hide">
                        <input type="checkbox" id="inspection_verified" name="inspection_verified" value="1" onclick="checkItems()"> 
                        Inspected, verified, and found in order as to quantity and specifications
                    </p>
                    <p class="hide">
                    <input type="checkbox" id="items_incomplete" name="items_incomplete" value="1" onclick="checkItems()"> 
                    Items Not Complete
                </p>
